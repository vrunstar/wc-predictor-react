...
from core.db import (
    get_est,
    fixtures_current_matchday, pred_updated,
    pred_map
)
...
FEATURES = BASE_DIR / 'model' / 'features.json'
...
def predict_today():
    ...
    fixtures = fixtures_current_matchday()

    if not fixtures:
        print(f"No Games Today {get_est()}")
        return

    for fixture in fixtures:
        preds = pred_map()
        exists = preds.get(fixture['match_id'])
        if exists:
